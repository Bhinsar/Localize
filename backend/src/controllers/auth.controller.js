...
exports.loginUser = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ user: data, token, refreshToken, existNumber, role, bios });
  } catch (error) {
...
exports.addNumberAndRole = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Number and role added successfully.", data });
  } catch (error) {
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    if (
      language.toLowerCase() !== "en" &&
      Object.keys(dataToTranslate).length > 0
    ) {
      try {
        const translatedData = await translateJsonData(
          dataToTranslate,
          language
        );
        ...
        if (translatedData) {
          if (providerDetails) {
            ...
            providerDetails.profiles.full_name =
              translatedData.full_name || providerDetails.profiles.full_name;
            providerDetails.services.name =
              translatedData.service_name || providerDetails.services.name;
          } else {
